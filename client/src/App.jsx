...
class ReviewApp extends React.Component {
  ...
  getRelevantReviews() {
    Parse.getAllList((data) => {
      ...
      ReactDOM.render(
        <MainReviewPanel
          reviews={this.state.reviewsToShow}
          meta={this.state.meta}
        />,
        document.getElementById('reviewPannel')
      );
    });
  }
  ...
  getNewestReviews() {
    Parse.getAllListNewest((data) => {
      ...
      ReactDOM.unmountComponentAtNode(document.getElementById('reviewPannel'));
      ReactDOM.render(
        <MainReviewPanel reviews={this.state.reviewsToShow} />,
        document.getElementById('reviewPannel')
      );
    });
  }
  ...
  render() {
    ...
    if (this.state[5]) {
      clearFiveStar = (
        <a value={5} onClick={() => this.handleClearFilter(5)}>
          <u>5 Stars</u> &nbsp;
        </a>
      );
    }
    ...
    if (this.state[4]) {
      clearFourStar = (
        <a value={4} onClick={() => this.handleClearFilter(4)}>
          <u>4 Stars</u> &nbsp;
        </a>
      );
    }
    ...
    if (this.state[3]) {
      clearThreeStar = (
        <a value={3} onClick={() => this.handleClearFilter(3)}>
          <u>3 Stars</u> &nbsp;
        </a>
      );
    }
    ...
    if (this.state[2]) {
      clearTwoStar = (
        <a value={2} onClick={() => this.handleClearFilter(2)}>
          <u>2 Stars</u> &nbsp;
        </a>
      );
    }
    ...
    if (this.state[1]) {
      clearOneStar = (
        <a value={1} onClick={() => this.handleClearFilter(1)}>
          <u>1 Star</u> &nbsp;
        </a>
      );
    }
    ...
    if (
      ...
      clearAll = (
        <a onClick={() => this.handleClearFilter()}>
          &nbsp;<u>Clear All</u>
        </a>
      );
    ...
    if (this.state.reviews.length === 0) {
      ...
      showMoreReviews = (
        <Button id='reviewButton' onClick={this.handleMoreReviews}>
          MORE REVIEWS{' '}
        </Button>
      );
    }
        ...
        <Container>
          ...
          <Row>
            ...
            <Col fluid>
              <Row id='reviewPannelHeader'>
                ...
                <Col id='sortDropdown' fluid>
                  <Row id='sort'>
                    <h3 id='sortTitle'>
                      {numberOfReviews}{' '}
                      {this.state.numberOfReviews === 1 ? 'review' : 'reviews'},
                      sorted by&nbsp;{' '}
                    </h3>
                    <DropdownButton
                      id='dropdown-item-button'
                      title={this.state.sortName}>
                      <Dropdown.Item
                        onClick={() => this.handleSort('helpfulness')}>
                        Helpfulness
                      </Dropdown.Item>
                      <Dropdown.Item
                        onClick={() => this.handleSort('relevance')}>
                        Relevance
                      </Dropdown.Item>
              ...
              <Row id='starFilter'>
                <p id='clearFilter'>
                  {filterTitle} {clearFiveStar} {clearFourStar} {clearThreeStar}{' '}
                  {clearTwoStar} {clearOneStar} {clearAll}
                </p>
              </Row>
