...
class SingleReview extends React.Component {
  ...
  render() {
    ...
    if (this.state.photos.length === 1) {
      photos = (
        <div>
          <p id='photos'>Photo:</p>
          <Image
            onClick={() => this.handlePhotoLightbox(0)}
            src={this.state.photos[0].url}
            thumbnail
            id='thumbnailReviewImage'
          />
        </div>
      );
      ...
      photos = (
        <div>
          ...
          />
          <Image
            onClick={() => this.handlePhotoLightbox(1)}
      ...
      photos = (
        <div>
          ...
          />
          <Image
            onClick={() => this.handlePhotoLightbox(2)}
      ...
      photos = (
        <div>
          ...
          />
          <Image
            onClick={() => this.handlePhotoLightbox(3)}
      ...
      photos = (
        <div>
          ...
          />
          <Image
            onClick={() => this.handlePhotoLightbox(4)}
    ...
    if (this.state.response) {
      response = (
        <Col id='response'>
          <p>Response from seller: {this.state.response}</p>
        </Col>
      );
    }
    ...
    if (this.state.didReport) {
      ...
      reported = (
        <u>
          <a className='report' onClick={this.handleReport}>
            Report
          </a>
        </u>
      );
    ...
    if (this.state.photoLightboxTriggered) {
      photoLightbox = (
        <Lightbox
          mainSrc={this.state.photos[this.state.lightboxId].url}
          onCloseRequest={() =>
            this.setState({ photoLightboxTriggered: false })
          }
        />
      );
    }
    ...
    return (
      <Container id='review' fluid>
        ...
        <Row>
          <Col id='body'>
            <p>
              {this.state.readMore ? this.state.body : this.state.bodyPreview}{' '}
              <a id='readMore' onClick={this.showMore}>
        ...
        <Row id='helpfulAndReport'>
          <p id='helpful'>
            Helpful? &nbsp;
            <u>
              <a className='helpful' onClick={this.handleClick}>
                Yes
              </a>
            </u>{' '}
            ({this.state.helpfulness}) &nbsp; &nbsp;| &nbsp; &nbsp; {reported}{' '}
            <br></br>
          </p>
        </Row>
      </Container>
